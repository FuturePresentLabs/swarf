...
use super::*;
use materials::{TOOL_DIAMETERS, get_engagement_factor};
...
pub fn compute_parameters(
    ...
    if engagement.radial_engagement_pct <= 0.0 || engagement.radial_engagement_pct > 100.0 {
        return Err(BlackBookError::InvalidEngagement(
            format!("Radial engagement must be 0-100%, got {}", engagement.radial_engagement_pct)
        ));
    }
    ...
    if material.high_feed_recommended && feed_rate_ipm < (rpm as f64 * base_chip_load * tool.flute_count as f64 * 0.5) {
        warnings.push(format!(
...
mod tests {
    ...
    fn test_chip_load_interpolation() {
        ...
        assert!(cl_3125 >= cl_250, "cl_3125={} should be >= cl_250={}", cl_3125, cl_250);
        assert!(cl_3125 <= cl_375, "cl_3125={} should be <= cl_375={}", cl_3125, cl_375);
    }
}
