...
impl BlackBook {
    ...
    pub fn get_chip_load(
        ...
        Ok(calculations::lookup_chip_load(material, tool_diameter, tool_material))
    }
...
mod tests {
    ...
    fn test_blackbook_initialization() {
        ...
        assert!(!materials.is_empty(), "Material database should not be empty");
        
        // Should have aluminum 6061
        assert!(materials.iter().any(|m| m.contains("6061")), "Should have 6061 aluminum");
    }
    ...
    fn test_aluminum_6061_calculation() {
        ...
        let params = bb.calculate("Aluminum 6061-T6", &tool, &engagement)
            .expect("Should calculate parameters for 6061");

        // 6061 should run fast
        assert!(params.rpm > 8000, "6061 should run > 8000 RPM with 1/4\" tool");
        assert!(params.sfm >= 800.0 && params.sfm <= 1500.0, 
            "6061 SFM should be 800-1500, got {}", params.sfm);
    }
    ...
    fn test_stainless_304_calculation() {
        ...
        let params = bb.calculate("Stainless 304", &tool, &engagement)
        ...
        assert!(params.sfm >= 100.0 && params.sfm <= 350.0,
            "304 SFM should be 100-350, got {}", params.sfm);
    }
    ...
    fn test_chip_thinning() {
        ...
        let low_params = bb.calculate("Aluminum 6061-T6", &tool, &low_engagement).unwrap();
        let high_params = bb.calculate("Aluminum 6061-T6", &tool, &high_engagement).unwrap();

        // Low engagement should have higher feed to maintain chip thickness
        assert!(low_params.feed_rate_ipm > high_params.feed_rate_ipm,
    ...
    fn test_titanium_calculation() {
        ...
        let params = bb.calculate("Titanium Ti-6Al-4V", &tool, &engagement)
    ...
    fn test_hss_vs_carbide() {
        ...
        let hss_params = bb.calculate("Aluminum 6061-T6", &hss_tool, &engagement).unwrap();
        let carbide_params = bb.calculate("Aluminum 6061-T6", &carbide_tool, &engagement).unwrap();

        // Carbide should run faster than HSS
        assert!(carbide_params.sfm > hss_params.sfm * 2.0,
    ...
    fn test_chip_load_lookup() {
        ...
        let small = bb.get_chip_load("Aluminum 6061-T6", 0.125, ToolMaterial::Carbide).unwrap();
        let large = bb.get_chip_load("Aluminum 6061-T6", 0.5, ToolMaterial::Carbide).unwrap();
